...
import static jp.monyone.library.DataStructure.RangeMinimumNthQueryNotSuccinctWaveletTree_Include.RMNthQ;
import static org.hamcrest.CoreMatchers.is;
import static org.junit.Assert.assertThat;

public class RangeMinimumNthQueryNotSuccinctWaveletTree_Test {
	// utility
		...
		for(int begin = 0; begin < array.length; begin++){
			for(int end = begin + 1; end <= array.length; end++){
				for(int kth = 1; kth <= (end - begin); kth++) {
					...
					nth.nth(begin, end, kth);
					assertThat(nth.nth(begin, end, kth), is(array[begin + (kth - 1)]));
				}
			}
		}
	}
	...
	@Test public void test_compare_actually_sort_in_range(){
		...
		for(int begin = 0; begin < array.length; begin++){
			for(int end = begin + 1; end <= array.length; end++){
				...
				for(int kth = 1; kth <= (end - begin); kth++){
					assertThat(nth.nth(begin, end, kth), is(sub_array[kth - 1]));
				}
			}
		}
	}
	...
	public static void POJ2014_Test(RMNthQ nth, int l, int r, int k, long answer) {
		assertThat(nth.nth(l - 1, r, k), is(answer));
	}
